...
用哈希表记下「原节点 → 克隆节点」的映射，再走一遍把 `next` / `random` 照着原表连上——思路最直白，代价是多一张表。
...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        # 2) 第二遍：照着原节点的 next / random 连接克隆节点
        cur = head
        while cur:
            if cur.next:
...
- 时间复杂度：`O(n)`，两遍遍历
- 空间复杂度：`O(n)`，额外的哈希表
- ⚠️ 多了一张 `O(n)` 的哈希表。把克隆节点直接插到原节点后面、用「原.random.next」就能就地找到对应克隆，省掉额外表，见下方最优解。
