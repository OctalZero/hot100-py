...
```mermaid
flowchart TD
    A["dummy 哨兵，prev 指向已处理尾"] --> B{"剩余 ≥ 2 个节点?"}
    B -->|"是"| C["取 first 与 second 两个节点"]
    C --> D["prev 指向 second"]
    D --> E["first 指向 second 的后继"]
    E --> F["second 指向 first"]
    F --> G["prev 移到 first，准备下一对"]
    G --> B
    B -->|"否"| H["返回 dummy.next"]
```

**总体一句话**：每次取相邻两个节点 `first`、`second`，用三个指针改向——`prev.next=second`、`first.next=second.next`、`second.next=first`，把这对翻转后接到已处理段尾部，再把 `prev` 移到新尾 `first` 继续下一对。

### 🔬 逐步推演（动画式）

以 `head = 1→2→3→4` 为例——从左到右就是算法的时间线：**每个节点是一次状态快照（交换后的链表形态与 prev 落点），箭头上写这一步交换了哪一对**：

```mermaid
flowchart LR
    S0(["初始<br/>dummy → 1→2→3→4　prev = dummy"]) -->|"交换 1 与 2<br/>2→1"| S1["dummy → 2→1→3→4<br/>prev = 节点 1"]
    S1 -->|"交换 3 与 4<br/>4→3"| Ans(["dummy → 2→1→4→3<br/>剩余不足一对 → 答案 = 2→1→4→3"])
```

读图：第 ① 步把 1、2 翻成 2→1，`prev` 落到新的尾 1；第 ② 步处理剩下的一对 3、4，翻成 4→3。剩余节点不足 2 个，结束。答案 **2→1→4→3**。
