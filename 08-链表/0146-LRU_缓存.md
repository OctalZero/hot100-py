...
```mermaid
flowchart TD
    A["操作 get 或 put"] --> B{"key 在哈希表?"}
    B -->|"命中"| C["取出节点：摘下 → 挂到表头"]
    B -->|"未命中 put"| D{"已达容量上限?"}
    D -->|"已满"| E["删除尾节点最久未使用 + 删哈希"]
    D -->|"未满"| F["新建节点挂到表头"]
    E --> F
    F --> G["写入哈希表"]
    C --> H["更新值 / 标记最近使用"]
...
**总体一句话**：用「哈希表 + 双向链表」组合——哈希表负责 `O(1)` 查找节点，双向链表维护「最近使用顺序」（表头最新、表尾最旧），每次访问就把节点挪到表头，满了就淘汰表尾。

### 🔬 逐步推演（动画式）

以容量 `capacity = 2` 为例，缓存用「左=表头最新 → 右=表尾最旧」表示——从左到右就是操作的时间线：**每个节点是一次状态快照（操作后的缓存状态），箭头上写了这一步是什么操作、命中还是淘汰**：

```mermaid
flowchart LR
    S0(["初始<br/>缓存空"]) -->|"put 1,1 未命中"| S1["缓存：1"]
    S1 -->|"put 2,2 未命中"| S2["缓存：2 → 1"]
    S2 -->|"get 1 命中，把 1 挪到表头"| S3["缓存：1 → 2"]
    S3 -->|"put 3,3 未命中且已满，淘汰尾节点 2"| S4["缓存：3 → 1"]
    S4 --> Ans(["get 2 未命中<br/>返回 -1"])
```
