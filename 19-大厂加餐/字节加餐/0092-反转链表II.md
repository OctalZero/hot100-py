...
flowchart TD
    ...
    B --> C["cur = prev.next，区间第一个节点"]
    C --> D{"还没做够 right-left 次?"}
    D -->|"是"| E["把 cur.next 摘下，头插到 prev 之后"]
    E --> D
    D -->|"否"| F["区间整体反转完成"]
```

**总体一句话**：用虚拟头 `dummy` 统一边界，`prev` 钉在区间前一个节点，反复把 `cur.next` 摘下来头插到 `prev` 后面——每次头插都把后一个节点搬到区间最前，循环 `right-left` 次区间就整体翻转。

### 🔬 逐步推演（动画式）

以 `head = 1→2→3→4→5`、`left = 2、right = 4` 为例——从左到右就是头插法的时间线：**每个节点是一次链表形态快照，箭头写这步摘了谁、插到哪**：

```mermaid
flowchart LR
    S0(["初始<br/>1→2→3→4→5<br/>prev→1, cur→2"]) -->|"摘 3，插到 1 后面"| S1["1→3→2→4→5"]
    S1 -->|"摘 4，插到 1 后面"| Ans(["结果 = 1→4→3→2→5<br/>区间 2,3,4 翻成 4,3,2"])
```
