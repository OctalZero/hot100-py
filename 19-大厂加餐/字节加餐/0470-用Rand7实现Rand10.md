...
```mermaid
flowchart TD
    A["初始化：进入循环"] --> B["两次 rand7 得 row、col"]
    B --> C["idx = row-1 乘 7 加 col，均匀覆盖 1~49"]
    C --> D{"idx ≤ 40 ?"}
    D -->|"否（41~49）"| E["拒绝，重新采样"]
    E --> B
    D -->|"是"| F["返回 idx-1 模 10 加 1"]
```

**总体一句话**：拒绝采样——两次 `rand7()` 拼出 `[1,49]` 的均匀分布，拒绝多余的 `41~49` 只保留 `[1,40]`（40 恰为 10 的倍数），再按 `(idx-1) % 10 + 1` 映射到 `[1,10]`，保证每个输出等概率。

### 🔬 逐步推演（动画式）

以某一次具体采样为例——从左到右就是算法的时间线：**每个节点是一次状态快照（row/col/idx 或结果），箭头上写这一步做了什么决策**：

```mermaid
flowchart LR
    S0(["初始<br/>进入采样循环"]) -->|"掷两次骰"| S1["row = 7　col = 2<br/>idx = 6×7+2 = 44"]
    S1 -->|"idx = 44 超过 40<br/>拒绝，重来"| S2["重新掷两次骰<br/>row = 3　col = 5<br/>idx = 2×7+5 = 19"]
    S2 -->|"idx = 19 ≤ 40<br/>接受"| S3["映射：18 模 10 + 1"]
    S3 --> Ans(["答案 = 9"])
```
