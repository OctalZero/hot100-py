...
**总体一句话**：标准 BFS 层序遍历 + 一个方向标志——左→右层原样收集，右→左层把本层结果反转一下，每处理完一层方向翻转，就能锯齿形读出整棵树。

### 🔬 逐步推演（动画式）

以 `root = [3,9,20,null,null,15,7]` 为例——从左到右就是逐层遍历的时间线：**每个节点是一层的遍历结果，箭头写这层往哪个方向收、方向是否翻转**：

```mermaid
flowchart LR
    S0(["初始<br/>队列 3<br/>方向 左→右"]) -->|"出队 3<br/>原序收集"| S1["本层 = 3<br/>方向翻为 右→左"]
    S1 -->|"入队 9, 20<br/>反转收集"| S2["本层 = 20,9<br/>方向翻为 左→右"]
    S2 -->|"入队 15, 7<br/>原序收集"| Ans(["结果 = 3 / 20,9 / 15,7<br/>第 1 层从右→左"])
```
