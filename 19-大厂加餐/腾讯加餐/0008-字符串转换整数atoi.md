...
```mermaid
flowchart TD
    A["初始化：指针 i=0, sign=1, res=0"] --> B["跳过前导空格"]
    B --> C{"s[i] 是符号?"}
    C -->|"是"| D["记录 sign, i 后移"]
    C -->|"否"| E
    D --> E{"s[i] 是数字?"}
    E -->|"否"| H["返回 sign 与 res 的乘积"]
    E -->|"是"| F["预判越界<br/>越界则钉死边界"]
    F --> G["res = res*10 + digit, i 后移"]
    G --> E
    H --> I["返回（已截断）"]
```

**总体一句话**：按题意走状态机——去空格、读符号、逐位累加成数字，每读一位都预判是否溢出、溢出就钉死在 INT_MAX / INT_MIN 边界，最后乘上符号返回。

### 🔬 逐步推演（动画式）

以 `s = '  -42'` 为例——从左到右就是算法的时间线：**每个节点是一次状态快照（指针 / 符号 / res），箭头上写这一步读了谁、做了什么决策**：

```mermaid
flowchart LR
    S0(["初始<br/>i=0　sign=1　res=0"]) -->|"跳过两个空格"| S1["i=2　sign=1　res=0"]
    S1 -->|"读到 '-'<br/>sign 置 -1, i 后移"| S2["i=3　sign=-1　res=0"]
    S2 -->|"读 '4'<br/>未越界, res=0*10+4"| S3["i=4　sign=-1　res=4"]
    S3 -->|"读 '2'<br/>未越界, res=4*10+2"| S4["i=5　sign=-1　res=42"]
    S4 -->|"到末尾, 非数字→停"| Ans(["答案 = -1 * 42 = -42<br/>未越界无需截断"])
```
