...
```mermaid
flowchart TD
    A["初始化：选定待排段 lo, hi"] --> B{"段长 > 1?"}
    B -->|"否"| G["直接返回"]
    B -->|"是"| C["随机选 pivot 换到 hi"]
    C --> D["Lomuto 划分：<br/>≤ pivot 的往前塞"]
    D --> E["pivot 落到最终位置 p"]
    E --> F["对左段、右段分别递归"]
    F --> G
```

**总体一句话**：每轮随机选一个基准做分区，让 pivot 一次性落到最终位置，左右两段天然独立、各自再排，分治到底就排好了。

### 🔬 逐步推演（动画式）

以 `nums = 3, 1, 2` 为例（随机 pivot 选到末尾的 2）——从左到右就是算法的时间线：**每个节点是一次分区后的数组快照，箭头上写这一轮分区做了什么**：

```mermaid
flowchart LR
    S0(["初始<br/>nums = 3, 1, 2"]) -->|"pivot=2<br/>划分：1 与 3 互换"| S1["分区后<br/>nums = 1, 2, 3<br/>pivot 落位 p=1"]
    S1 -->|"左段 1, 2 里只剩单元素"| S2["左段结束"]
    S2 -->|"右段 3 只剩单元素"| Ans(["答案 = 1, 2, 3<br/>升序完成"])
```
