...
```mermaid
flowchart TD
    A["初始化：lo=0, hi=n-1（闭区间）"] --> B{"lo ≤ hi?"}
    B -->|"否"| C["返回 -1（没找到）"]
    B -->|"是"| D["mid = lo 与 hi 的中点"]
    D --> E{"nums[mid] 与 target?"}
    E -->|"相等"| F["返回 mid"]
    E -->|"小于"| G["lo = mid+1（去右半）"]
    E -->|"大于"| H["hi = mid-1（去左半）"]
    G --> B
...
**总体一句话**：数组有序 → 每次取闭区间中点比较，相等即命中、比中点小去左半、比中点大去右半，每次砍掉一半，O(log n) 锁定 target。

### 🔬 逐步推演（动画式）

以 `nums = -1, 0, 3, 5, 9, 12`，`target = 9` 为例——从左到右就是算法的时间线：**每个节点是一次三指针快照（lo / mid / hi），箭头上写比较结果与如何收缩**：

```mermaid
flowchart LR
    S0(["初始<br/>lo=0　mid=2　hi=5<br/>nums[mid]=3"]) -->|"3 < 9<br/>去右半, lo=mid+1"| S1["lo=3　mid=4　hi=5<br/>nums[mid]=9"]
    S1 -->|"9 == 9<br/>命中"| Ans(["答案 = 4<br/>target 在下标 4"])
```
