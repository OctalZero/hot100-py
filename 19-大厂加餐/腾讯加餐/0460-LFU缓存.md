...
```mermaid
flowchart TD
    A["初始化：三张哈希 + min_freq"] --> B{"操作类型?"}
    B -->|"get 命中"| C["频次 +1：<br/>旧桶队尾挪到新桶队尾"]
    B -->|"put 已存在"| D["更新值 + 频次 +1"]
    B -->|"put 新 key"| E{"容量已满?"}
    E -->|"满"| F["淘汰 min_freq 桶队头<br/>最久未用"]
    E -->|"未满"| G
    F --> G["放 freq=1 桶队尾<br/>min_freq 重置为 1"]
    C --> H["返回 / 完成"]
    D --> H
    G --> H
```

**总体一句话**：把每个 key 按「使用频次」分桶，桶内用 OrderedDict 维护 LRU（队头最旧、队尾最新）——访问即「频次+1 = 从旧桶挪到新桶队尾」，淘汰即「抓 min_freq 桶队头」，全程哈希 O(1)。

### 🔬 逐步推演（动画式）

以 `capacity = 2`，依次 `put 1,1 → put 2,2 → get 1 → put 3,3` 为例——从左到右就是操作的时间线：**每个节点是一次缓存内部状态快照（各频次桶 + min_freq），箭头上写这次操作做了什么**：

```mermaid
flowchart LR
    S0(["初始<br/>空, min_freq=0"]) -->|"put 1,1<br/>新 key, freq=1"| S1["freq1: 1<br/>min_freq=1"]
    S1 -->|"put 2,2<br/>新 key, freq=1"| S2["freq1: 1, 2<br/>min_freq=1"]
    S2 -->|"get 1<br/>1 频次 1→2"| S3["freq1: 2<br/>freq2: 1<br/>min_freq=1"]
    S3 -->|"put 3,3<br/>满, 淘汰 min_freq 桶队头=2"| Ans(["freq1: 3<br/>min_freq=1<br/>2 已淘汰, 后续 get 2 返回 -1"])
```
