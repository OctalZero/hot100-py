...
```mermaid
flowchart TD
    A["初始化：原数组"] --> B["建大顶堆<br/>非叶子节点逐个 sift down"]
    B --> C["堆顶即全局最大"]
    C --> D["堆顶 ↔ 末尾<br/>最大值归位"]
    D --> E["缩小堆边界<br/>对新堆顶 sift down"]
    E --> F{"堆里还有 >1 个?"}
    F -->|"是"| C
    F -->|"否"| G["数组升序完成"]
```

**总体一句话**：先把数组原地建成大顶堆（最大值在根），再反复「堆顶与末尾交换、缩小堆、对新堆顶下沉」——每次都把当前最大值钉到末尾，从后往前填出升序。

### 🔬 逐步推演（动画式）

以 `nums = 4, 2, 5, 1, 6, 3` 为例——从左到右就是算法的时间线：**每个节点是一次数组快照，箭头上写这一步做了什么**：

```mermaid
flowchart LR
    S0(["初始<br/>nums = 4, 2, 5, 1, 6, 3"]) -->|"自下而上建大顶堆<br/>sift down 重建堆序"| S1["建堆后<br/>nums = 6, 4, 5, 1, 2, 3"]
    S1 -->|"堆顶 6 ↔ 末尾<br/>sift down 恢复堆"| S2["nums = 5, 4, 3, 1, 2, 6"]
    S2 -->|"堆顶 5 ↔ 末尾<br/>sift down 恢复堆"| S3["nums = 4, 2, 3, 1, 5, 6"]
    S3 -->|"继续换堆顶到末尾<br/>逐轮 sift down"| Ans(["答案 = 1, 2, 3, 4, 5, 6<br/>升序完成"])
```
