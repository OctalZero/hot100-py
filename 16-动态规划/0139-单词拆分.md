...
```mermaid
flowchart TD
    A["dp0 = True 空串可拆"] --> B{"每个位置 i?"}
    B -->|"有"| C["枚举切点 j < i"]
    C --> D{"dp[j] 为真 且 s 的 j~i 段在词典?"}
    D -->|"是"| E["dp[i] = True，跳到下一个 i"]
    D -->|"否"| C
    E --> B
    B -->|"无"| F["返回 dp[n]"]
```

**总体一句话**：`dp[i]` 表示前 i 个字符能否拆分——枚举切点 j，只要 `dp[j]` 为真且子串 `s 的 j~i 段` 在词典里，`dp[i]` 就为真；空串 `dp[0]` 预置真。

### 🔬 逐步推演（动画式）

以 `s = "leetcode"`、词典 = leet, code 为例——从左到右就是算法的时间线：**每个节点是一次状态快照（dp 表的关键位），箭头上写这一步在哪一刀切、怎么判定**：

```mermaid
flowchart LR
    S0(["初始<br/>dp0 = 真　其余假"]) -->|"i=4 切点 j=0<br/>leet 在词典"| S1["dp4 = 真"]
    S1 -->|"i=8 切点 j=4<br/>code 在词典"| Ans(["答案 = 真<br/>leet + code 拼成整串"])
```
...
从起点开始，尝试用字典里的每个单词去匹配前缀，匹配上就递归处理剩余后缀——不记忆化，存在大量重复计算。
...
- 时间复杂度：`O(2ⁿ)`，最坏情况下每个切点都递归展开，呈指数级
- 空间复杂度：`O(n)`，递归栈深度
- ⚠️ n 一大就超时。发现「同一个起点是否可拆」会被反复求解 → 加张 dp 表记忆化，演进到下方 `O(n²)` 的动态规划。
