...
```mermaid
flowchart TD
    A["初始化 cur_max = cur_min = ans = 首元素"] --> B{"还有下一个 num?"}
    B -->|"有"| C["暂存旧 cur_max"]
    C --> D["cur_max = num、旧 max*num、旧 min*num 三者取大"]
    D --> E["cur_min = 同三者取小"]
    E --> F["ans = ans 与 cur_max 的较大者"]
    F --> B
    B -->|"无"| G["返回 ans"]
```

**总体一句话**：乘法里负数会翻转大小，所以同时维护以当前位置结尾的「最大乘积 cur_max」和「最小乘积 cur_min」，每来一个 num 就在 num、旧 max×num、旧 min×num 三者中分别取大/取小更新，全局最大即为答案。

### 🔬 逐步推演（动画式）

以 `nums = 2, 3, -2, 4` 为例——从左到右就是算法的时间线：**每个节点是一次状态快照（cur_max / cur_min / ans），箭头上写这一步处理了谁、怎么算**：

```mermaid
flowchart LR
    S0(["初始<br/>max = 2　min = 2　ans = 2"]) -->|"处理 3<br/>max = 三者取大 = 6"| S1["max = 6　min = 3　ans = 6"]
    S1 -->|"处理 -2<br/>负数翻转 → 另起最大"| S2["max = -2　min = -12　ans = 6"]
    S2 -->|"处理 4<br/>4 最大 → 另起"| Ans(["答案 = 6<br/>最大子数组为 2,3"])
```
