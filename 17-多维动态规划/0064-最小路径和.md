...
```mermaid
flowchart TD
    A["初始化 dp[0][0] = grid[0][0]"] --> B["首行只能从左来、首列只能从上，各自累加"]
    B --> C["逐格遍历内部位置"]
    C --> D["dp[i][j] = grid[i][j] + min(上, 左)"]
    D --> C
    C -->|"遍历完"| E["返回 dp[m-1][n-1]"]
```

**总体一句话**：每个格子的最小累计 = 自己的花费 +「从上来」和「从左来」中较小的那一份——边界只能一条路走进来，单独累加。

### 🔬 逐步推演（动画式）

以 `grid = [[1,3,1],[1,5,1],[4,2,1]]` 为例——从左到右就是算法填表的时间线：**每个节点是一次状态快照（已填好的 dp 表，未填处用 . 表示），箭头上写这一步填哪个格子、怎么算**：

```mermaid
flowchart LR
    S0(["首行首列累加<br/>1 4 5<br/>2 . .<br/>6 . ."]) -->|"填 (1,1)<br/>5 + min(2,4)=7"| S1["1 4 5<br/>2 7 .<br/>6 . ."]
    S1 -->|"填 (1,2)<br/>1 + min(7,5)=6"| S2["1 4 5<br/>2 7 6<br/>6 . ."]
    S2 -->|"填 (2,1)<br/>2 + min(6,7)=8"| S3["1 4 5<br/>2 7 6<br/>6 8 ."]
    S3 -->|"填 (2,2)<br/>1 + min(8,6)=7"| S4["1 4 5<br/>2 7 6<br/>6 8 7"]
    S4 --> Ans(["答案 = 7<br/>路径 1→3→1→1→1"])
```
