...
🔗 [LeetCode 98](https://leetcode.cn/problems/validate-binary-search-tree/description/?envType=study-plan-v2&envId=top-100-liked)

## 🛒 人话理解 & 🧠 思路演进

```mermaid
flowchart TD
    A["初始化：栈空，指针指向根，prev = 负无穷"] --> B{"指针非空 或 栈非空?"}
    B -->|"是"| C["一路向左压栈"]
    C --> D["弹栈顶 node"]
    D --> E{"node.val > prev?"}
    E -->|"是，递增"| F["prev = node.val，转右子树"]
    E -->|"否，非递增"| G["不是 BST ✗"]
    F --> B
    B -->|"否（都空）"| H["是 BST ✅"]
```

**总体一句话**：BST 的中序遍历必然严格递增，所以边中序遍历边比较「当前值是否严格大于上一个值」即可——出现一次 ≤ 即判否，全程递增则判是。

### 🔬 逐步推演（动画式）

以 `root = [2,1,3]`（根 2，左 1，右 3）为例——从左到右就是中序遍历比较的时间线：**每个节点是一次状态快照（弹出的值 / prev），箭头上写这一步是否严格递增**：

```mermaid
flowchart LR
    S0(["初始<br/>prev = 负无穷"]) -->|"弹 1<br/>1 > 负无穷 递增"| S1["prev = 1"]
    S1 -->|"弹 2<br/>2 > 1 递增"| S2["prev = 2"]
    S2 -->|"弹 3<br/>3 > 2 递增"| Ans(["答案 = 是 BST<br/>1→2→3 全程递增"])
```
...
BST 的中序遍历必然严格递增——朴素做法：先中序把所有值读进数组，再线性扫一遍验证是否严格递增。
...
- 时间复杂度：`O(n)`，每个节点访问一次
- 空间复杂度：`O(n)`，额外用一个数组保存整棵树的中序序列
- ⚠️ 多开了 O(n) 数组。其实递归过程中只要记住「上一个值」就能即时比较，不必存全部——演进到下方 O(h) 空间的区间递归解。
