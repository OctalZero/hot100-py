...
```mermaid
flowchart TD
    A["初始化前缀计数 cnt = 0→1"] --> B["向下递归, currSum 累加 node.val"]
    B --> C["查 currSum - target 的出现次数"]
    C --> D["路径数累加该次数"]
    D --> E["currSum 计数 +1"]
    E --> F["递归左、右子树"]
    F --> G["回溯, currSum 计数 -1"]
...
**总体一句话**：沿根到当前节点维护前缀和 `currSum`，若某祖先处的前缀和等于 `currSum - target`，则该祖先到当前节点的这段路径和恰为 `target`——用哈希表记录各前缀和出现次数，一次遍历即可统计。

### 🔬 逐步推演（动画式）

以 `root = [10,5,-3,3,2,null,11,3,-2,null,1]`、`target = 8` 为例——沿根到当前节点维护前缀和 `currSum`，**每个节点是一次状态快照（currSum 与累计命中数），箭头上写访问了谁、查 currSum-8 命中几次**（此处沿左链 `10 → 5 → 3` 演示一次命中，其余分支同理再命中 2 条）：

```mermaid
flowchart LR
    S0(["初始<br/>哈希表 0:1 / ans = 0"]) -->|"访问 10<br/>查 10-8=2 未命中"| S1["currSum = 10<br/>ans = 0"]
    S1 -->|"访问 5<br/>查 15-8=7 未命中"| S2["currSum = 15<br/>ans = 0"]
    S2 -->|"访问 3<br/>查 18-8=10 命中 1 次"| S3["currSum = 18<br/>ans = 1"]
    S3 -->|"其余分支再命中 2 条"| Ans(["答案 = 3<br/>5→3 / 5→2→1 / -3→11"])
```
