...
**总体一句话**：前序首元素必是根，到中序里定位它就能切出左右子树区间，再递归构造——前序决定「谁是根」，中序决定「左右各有多少」。

### 🔬 逐步推演（动画式）

以 `preorder = [3,9,20,15,7]`、`inorder = [9,3,15,20,7]`（哈希表 `{9:0,3:1,15:2,20:3,7:4}`）为例——从左到右就是递归切根的时间线：**每个节点是一次状态快照（当前根 / 切出的左右区间），箭头上写这一步怎么定位根、怎么切**：

```mermaid
flowchart LR
    S0(["初始<br/>前序首 = 3"]) -->|"中序下标 k=1<br/>左 9 / 右 15,20,7"| S1["根 = 3<br/>建左子 树"]
    S1 -->|"前序首 = 9<br/>k=0 两边都空"| S2["根 = 9 叶子"]
    S2 -->|"前序首 = 20<br/>k=3 左 15 / 右 7"| S3["根 = 20<br/>左 15 右 7"]
    S3 -->|"15、7 各为叶子<br/>区间为空返 null"| Ans(["答案 = 3 / 左9 / 右20→15,7"])
```
...
最直白的递归分治：前序第一个必是根，到中序里线性查找它来切出左右子树区间，再递归构造——每层都 `.index()` 线性扫一遍、还要切片拷贝。
...
- 时间复杂度：`O(n²)`，每层 `.index()` 线性查找根，最坏递归 n 层
- 空间复杂度：`O(n²)`，每层切片都额外拷贝子数组
- ⚠️ 查找根和切片拷贝都是重复劳动。给中序建一张「值 -> 下标」哈希表，再用闭区间指针代替切片，就能把时间降到 O(n)、省掉所有拷贝——演进到下方最优解。
