...
```mermaid
flowchart TD
    A["初始化 candidate、count = 0"] --> B{"还有下一个 num?"}
    B -->|"有"| C{"count == 0?"}
    C -->|"是"| D["换候选 candidate = num"]
    C -->|"否"| E{"num == candidate?"}
    E -->|"同党"| F["count + 1"]
    E -->|"异党"| G["count - 1"]
    D --> H["count + 1"]
    ...
    G --> B
    H --> B
    B -->|"遍历完"| I["返回 candidate：多数派最后留下"]
```

**总体一句话**：摩尔投票把数组当成对拼消耗——同党 `count+1`、异党 `count-1`，`count` 归零就换候选；多数派（超过一半）人头比其余所有派系加起来还多，对拼到最后留下的一定是它，O(n)、O(1)。

### 🔬 逐步推演（动画式）

以 `nums = 2,2,1,1,2` 为例（多数元素是 2）——从左到右就是投票对拼的时间线：**每个节点是 candidate 与 count 的当前值，箭头上写遇到的是同党还是异党**：

```mermaid
flowchart LR
    S0(["初始<br/>candidate=无　count=0"]) -->|"遇 2<br/>count 归零→换候选"| S1["candidate=2　count=1"]
    S1 -->|"遇 2<br/>同党"| S2["candidate=2　count=2"]
    S2 -->|"遇 1<br/>异党消耗"| S3["candidate=2　count=1"]
    S3 -->|"遇 1<br/>异党消耗→count 归零"| S4["candidate=2　count=0"]
    S4 -->|"遇 2<br/>count 归零→换候选"| Ans(["答案 = 2<br/>多数派 2 最后留下"])
```
