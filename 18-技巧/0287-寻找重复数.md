...
```mermaid
flowchart TD
    A["值当下标→把数组看成链表"] --> B["快慢指针同点起步"]
    B --> C["slow 走一步、fast 走两步"]
    C --> D{"slow == fast?"}
    D -->|"否"| C
    D -->|"环内相遇"| E["slow 拉回起点"]
    E --> F["两指针都改走一步"]
    F --> G{"slow == fast?"}
    G -->|"否"| F
    G -->|"再次相遇"| H["相遇点 = 环入口 = 重复数"]
```

**总体一句话**：把下标 i 的「下一个节点」当成 `nums[i]`，数组就变成一条带环链表——两个下标指向同一个值形成环，**环的入口正是重复的数**；用快慢指针先在环内相遇、再把 slow 拉回起点同步走，第二次相遇点即为入口，O(n)、O(1) 且不修改原数组。

### 🔬 逐步推演（动画式）

以 `nums = 1,3,4,2,2` 为例（链表形态 `1→3→2→4→2` 成环于节点 2，重复数是 2）——从左到右就是快慢指针的时间线：**每个节点是 slow 与 fast 当前所在的链表节点，箭头上写走几步、是否相遇**：

```mermaid
flowchart LR
    S0(["起点<br/>slow=1　fast=1"]) -->|"slow 走1 fast 走2<br/>1→3　1→3→2"| S1["slow=3　fast=2"]
    S1 -->|"slow 走1 fast 走2<br/>3→2　2→4→2"| S2["slow=2　fast=2<br/>环内相遇"]
    S2 -->|"slow 拉回起点<br/>fast 留在 2"| S3["slow=1　fast=2"]
    S3 -->|"两者都走1步<br/>1→3　2→4"| S4["slow=3　fast=4"]
    S4 -->|"再各走1步<br/>3→2　4→2"| Ans(["答案 = 2<br/>环入口即重复数"])
```
...
用集合记录已出现过的数，遍历时第一次遇到「已在集合中」的就是重复数——思路最直白。
...
- 时间复杂度：`O(n)`
- 空间复杂度：`O(n)`，集合要存元素
- ⚠️ 不满足题目「只用常量 O(1) 额外空间」的要求，仅作思路对照。把数组当成链表、用快慢指针找环入口，可把空间压到 `O(1)` 且不修改原数组 → 演进到下方弗洛伊德判圈解。
