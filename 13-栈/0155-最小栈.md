...
```mermaid
flowchart TD
    A["主栈 push x"] --> B{"x 与 辅助栈顶 谁小?"}
    B -->|"x 不更大 → 同步入栈"| C["辅助栈 push x"]
    B -->|"x 更大 → 不动"| D["辅助栈不变"]
    C --> E["getMin 直接看辅助栈顶"]
    D --> E
    A --> F["pop 时若弹出的值等于辅助栈顶 → 辅助栈同步弹出"]
    F --> E
```

**总体一句话**：主栈正常存元素，另开一个「同步最小值」辅助栈——`push` 时若新值不大于当前最小就同步入栈，`pop` 时若弹出的恰好是最小就同步弹出，`getMin` 直接取辅助栈顶，全程 `O(1)`。

### 🔬 逐步推演（动画式）

以 `push(-2)、push(0)、push(-3)、getMin、pop、getMin` 为例——从左到右就是操作时间线：**每个节点是一次「主栈 / 辅助栈」状态快照，箭头上写这一步做了什么操作、为何更新或不动辅助栈**：

```mermaid
flowchart LR
    S0(["初始<br/>主 = 空　辅助 = 空"]) -->|"push -2<br/>辅助空 → 同步入"| S1["主 = -2　辅助 = -2"]
    S1 -->|"push 0<br/>0 不小于 -2 → 不入"| S2["主 = -2, 0　辅助 = -2"]
    S2 -->|"push -3<br/>-3 小于 -2 → 同步入"| S3["主 = -2, 0, -3　辅助 = -2, -3"]
    S3 -->|"getMin<br/>看辅助栈顶"| S4["返回 -3"]
    S4 -->|"pop -3<br/>弹出值等于辅助顶 → 同步弹"| S5["主 = -2, 0　辅助 = -2"]
    S5 --> Ans(["getMin → -2<br/>辅助栈顶即当前最小"])
```
