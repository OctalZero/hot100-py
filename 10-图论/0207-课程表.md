...
**总体一句话**：拓扑排序——反复把「入度为 0（无前置依赖）」的课程拿掉，拿掉后新出现的入度 0 课程继续拿；最后看拿掉的课数是否等于总课数，相等则无环、可完成，否则有环。

### 🔬 逐步推演（动画式）

以 `numCourses = 4, prerequisites = [[1,0],[2,1],[3,2]]`（链式依赖 `0→1→2→3`）为例——**每个节点是一次状态快照（队列 + 已学数），箭头上写这一步学掉哪门课、谁的入度归零入队**：

```mermaid
flowchart LR
    S0(["初始<br/>入度 0/1/1/1<br/>队列 = 0<br/>已学数 = 0"]) -->|"学掉 0，后继 1 入度 1→0 入队"| S1["队列 = 1<br/>已学数 = 1"]
    S1 -->|"学掉 1，后继 2 入度 1→0 入队"| S2["队列 = 2<br/>已学数 = 2"]
    S2 -->|"学掉 2，后继 3 入度 1→0 入队"| S3["队列 = 3<br/>已学数 = 3"]
    S3 -->|"学掉 3，无后继<br/>队列空"| Ans(["答案 = true<br/>已学数 4 = 总数 4"])
```
...
不用队列、不做 BFS：每一轮都重新扫描所有课程，把「当前入度为 0 且尚未选修」的课程挑出来学掉，再回写依赖关系，直到某一轮学不动为止——思路最朴素，但每轮都要全量扫一遍。
...
class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        ...
        learned = 0
        # 反复扫描：每一轮把入度已为 0 的课全部学掉
        while True:
            new_learned = False
            for i in range(numCourses):
                if in_degree[i] == 0:
                    in_degree[i] = -1  # 标记为已学，避免重复挑中
                    learned += 1
                    new_learned = True
                    # 学完 i 后，依赖 i 的课入度 -1
                    for course, prereq in prerequisites:
                        if prereq == i:
                            in_degree[course] -= 1
            if not new_learned:  # 这一轮没有任何课可学 → 结束
                break

        return learned == numCourses
```

- 时间复杂度：`O(N × (N + E))`，最坏每轮只学一门课，要扫 N 轮，每轮遍历 N 门课和 E 条依赖
- 空间复杂度：`O(N)`
- ⚠️ 每轮全量重扫做了大量重复工作。观察到「入度变为 0 的课」可以即时入队 → 演进到下方 `O(N + E)` 的 BFS 拓扑排序。
