...
```mermaid
flowchart TD
    A["初始化 根节点 root（空）"] --> B["按字符逐层向下"]
    B --> C{"当前节点的子节点中有该字符?"}
    C -->|"否"| D["新建子节点"]
    C -->|"是"| E["走到该子节点"]
    D --> E
    E --> B
    B -->|"字符走完"| F{"是 insert 操作?"}
    F -->|"是"| G["末节点标记 is_end = True"]
    F -->|"否 查询"| H["返回末节点是否存在 + 是否需 is_end"]
```

**总体一句话**：把字符串按字符逐层挂到一棵树上，公共前缀共享同一条路径；插入时沿路建节点并在末尾打 `is_end` 标记，查询时沿路找末节点，`search` 还要额外确认 `is_end`，`startsWith` 只确认末节点存在。

### 🔬 逐步推演（动画式）

以依次 `insert("apple")`、`insert("app")`、`search("app")`、`search("appl")` 为例——**每个节点是一次状态快照（树的形态，✱表示 is_end=True），箭头上写这一步做了什么操作、走了哪些字符**：

```mermaid
flowchart LR
    S0(["初始<br/>空树，只有 root"]) -->|"insert apple<br/>a→p→p→l→e 一路向下建节点，末尾打标"| S1["root→a→p→p→l→e✱"]
    S1 -->|"insert app<br/>复用 a→p→p，末节点 p 打标"| S2["root→a→p→p✱→l→e✱"]
    S2 -->|"search app<br/>走到 p，is_end=True"| S3["命中标记<br/>返回 True"]
    S3 -->|"search appl<br/>走到 l，is_end=False"| Ans(["答案 = False<br/>走到节点但非完整单词"])
```
...
不造树，直接用 Python 现成的 `set` 把所有单词存起来：`search` 用 `in`，`startsWith` 暴力遍历所有单词逐个判前缀。代码最短，但查询慢。

```python
class Trie:
    def __init__(self):
        self.words = set()  # 直接用集合存所有插入过的单词

    def insert(self, word: str) -> None:
        self.words.add(word)

    def search(self, word: str) -> bool:
        return word in self.words  # O(L) 算哈希

    def startsWith(self, prefix: str) -> bool:
        # 暴力：遍历每个单词，看是否以 prefix 开头
        for w in self.words:
...
- 时间复杂度：`insert` `O(L)`；`search` `O(L)`；`startsWith` `O(N·L)`，N 为已存单词数（每个都要做一次前缀比较）
- 空间复杂度：`O(总字符数)`
- ⚠️ `startsWith` 要逐个比对所有单词，单词一多就慢。观察到「公共前缀」可以共享路径 → 演进到下方前缀树，三种操作都 `O(L)`。
