...
```mermaid
flowchart TD
    A["从 start 处继续切"] --> B["枚举切点 end 从 start+1 到末尾"]
    B --> C{"前缀 s 的 start 到 end 是回文?"}
    C -->|"否"| D["延长前缀：换下一个 end"]
    C -->|"是"| E["把前缀加入 path"]
    D --> B
    E --> F["递归处理剩余，start 更新为 end"]
    F --> G{"start 到末尾 ?"}
    G -->|"到末尾"| H["收一种切分方案"]
    G -->|"未到末尾"| A
    H --> I["回溯：弹出前缀"]
...
**总体一句话**：从左到右扫，枚举每个切点 `end`，只要前缀是回文就切下来收进 `path` 并递归切剩余部分，切到串尾收一种方案，回来弹出前缀再试更长的前缀。

### 🔬 逐步推演（动画式）

以 `s = aab` 为例，跟踪切点 `end` 和当前 `path` 怎么长又怎么缩，从左到右就是回溯的时间线——每个节点是一次 `path` 快照，箭头上写这一步切下了哪段、是不是回文：

```mermaid
flowchart LR
    S0(["start=0<br/>前缀 a 是回文<br/>path = a"]) -->|"start=1 前缀 a 回文<br/>切下"| S1["path = a,a"]
    S1 -->|"start=2 前缀 b 回文<br/>切到串尾"| S2["path = a,a,b<br/>✅ 收方案一"]
    S2 -->|"逐层弹出 b、a<br/>start=0 试前缀 aa"| S3["前缀 aa 回文<br/>path = aa"]
    S3 -->|"start=2 前缀 b 回文<br/>切到串尾"| Ans(["答案<br/>方案一 a,a,b<br/>方案二 aa,b　共 2 种"])
```
